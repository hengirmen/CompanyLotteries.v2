...
describe("CompanyLotteries - checkIfAddressTicketWon", function () {
  ...
  describe('Successful checkIfAddressTicketWon', function () {
    it('should return a boolean value if the ticket won or not', async function () {
      await ethers.provider.send('evm_increaseTime', [1801]); // 30 minutes later
      await ethers.provider.send('evm_mine'); // Ensure time increment
      ...
      const buyerTicketWon = await companyLotteries.checkIfAddressTicketWon(
        buyer.address, // address
        lotteryNo, // lottery no
        1 // ticket no
      );

      expect(buyerTicketWon).to.be.a('boolean');
    });
  });

  describe('Failed checkIfAddressTicketWon', function () {
    ...
    it('should revert if the ticket does not exist or is unowned', async function () {
      await ethers.provider.send('evm_increaseTime', [1801]); // 30 minutes later
      await ethers.provider.send('evm_mine'); // Ensure time increment
      ...
      await expect(
        companyLotteries.checkIfAddressTicketWon(
          buyer.address, // address
          lotteryNo, // lottery no
          99 // Non-existent ticket no
        )
      ).to.be.revertedWith('Ticket does not exist or is unowned!');
    });

    it('should revert if the ticket does not belong to the address', async function () {
      await ethers.provider.send('evm_increaseTime', [1801]); // 30 minutes later
      await ethers.provider.send('evm_mine'); // Ensure time increment
      ...
      ).to.be.revertedWith('Ticket does not belong to the address!');
    });
  });
});
